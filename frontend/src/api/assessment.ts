...
const transition = (id: number, target_status: PlanStatus, version: number, reason: string, action: 'submit' | 'approve' | 'return') => request<DecompressionAssessment>(`/assessments/${id}/${action}`, { method: 'POST', body: JSON.stringify({ target_status, version, reason }) })
...
export const returnAssessment = (id: number, version: number, reason: string) => transition(id, 'draft', version, reason, 'return')
