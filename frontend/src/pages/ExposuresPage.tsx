...
import { Activity, Layers3, RotateCcw, Timer, Waves } from 'lucide-react'
...
import { isAwaitingSegmentChange, isSuperseded } from '@/utils/assessment'
...
export function ExposuresPage() {
  ...
  const latest = assessments.items.find((item) => !isSuperseded(item)) ?? assessments.items[0] ?? null
  const selectedPlan = plans.items.find((plan) => plan.id === planId) ?? plans.selected
  const awaitingSegmentChange = selectedPlan ? isAwaitingSegmentChange(selectedPlan) : false
  ...
  return (
    <div className="page">
      ...
      {selectedPlan && <div className="exposure-strip"><div><Waves size={19} /><span>PLAN<strong>{selectedPlan.plan_code}</strong></span></div><div><Timer size={19} /><span>ELAPSED<strong>{elapsed.toFixed(1)} min</strong></span></div><div><Activity size={19} /><span>MAX DEPTH<strong>{maxDepth.toFixed(1)} m</strong></span></div><div><Layers3 size={19} /><span>MODEL<strong>{latest && !isSuperseded(latest) ? latest.algorithm_version : 'AWAITING RERUN'}</strong></span></div><PlanStatusBadge status={selectedPlan.plan_status} /></div>}
      {awaitingSegmentChange && <div className="gate-banner"><RotateCcw size={16} /><span>The latest assessment was returned to draft at input v{selectedPlan?.rerun_gate_version}. Its preserved snapshot stays read-only; modify an exposure segment and rerun the model before a new review can start.</span></div>}
      {latest && isSuperseded(latest) && !awaitingSegmentChange && <div className="gate-banner"><RotateCcw size={16} /><span>Showing the preserved snapshot of returned assessment #{latest.id}; it can no longer be submitted or approved.</span></div>}
